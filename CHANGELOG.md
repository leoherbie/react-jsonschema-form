...
- Fixed `sanitizeDataForNewSchema()` leaving an `object` or `array` property holding the old subschema's `default` when switching between `oneOf`/`anyOf` options. The comparison that replaces a stale value only ran for scalar properties; object and array ones were recursed into instead, so a nested object, array or array of objects kept the previous option's default. The value is now compared against the default the old schema would have *computed* (children's own defaults merged in, as `getDefaultFormState()` produces it) rather than the literal `default` keyword, and a default that sits on an array's `items` is applied per element, fixing [#4476](https://github.com/rjsf-team/react-jsonschema-form/issues/4476)
- Changed `sanitizeDataForNewSchema()` to leave a value alone when the constraint it is being measured against did not change between the two schemas, so a property the option switch never touched keeps its data: a `readOnly` field holding a server-supplied value that differs from its schema `default` is no longer cleared on every switch, and an `object` or `array` value is no longer cleared or substituted by a `const` or enum-like constraint that neither schema changed. A scalar's `const` and `enum` are still applied unconditionally, since a dependency can narrow them without the property's own schema changing. `undefined`-valued keys are disregarded when comparing a value against a `default` or `const`, since this function writes such keys itself when clearing data
...
- Documented the `compileSchemaValidators` and `createPrecompiledValidator` APIs on the `validator-ata` API reference page ([#5078](https://github.com/rjsf-team/react-jsonschema-form/pull/5078))
...
- Removed the `ataValidatorBrowserCompat` workaround from the playground Vite config. The bump to `ata-validator` `^0.18.2` makes the patch unnecessary, since the validator no longer touches the filesystem at module init. See [ata-core/ata-validator#25](https://github.com/ata-core/ata-validator/issues/25)
